...
작성일: 2026-07-11 · 개정: 2026-07-15 (v0.4-draft, §0.10 구현 정합 반영) · 상태: **구현 반영** — §0.2 기본값은 대부분 구현·운영 중이며 잔여 확정 항목(A1 소셜·A10)만 표기.
...
### 0.2 기본값 — 현재 구현·운영 중 (일부 잔여 ⚠️)

> v0.1에서 "가정값"으로 열어둔 항목들은 **대부분 그대로 구현·운영 중**이다. 아래 `현황` 열이 실제 동작 상태이며, `⚠️`가 붙은 **A1(소셜)·A10만 잔여 확정 대상**이다. `현황` 범례: **✅ 운영 중** · **⚠️ 부분(caveat)** · **⭕ 선택·미구현**.

| #   | 항목           | 현황 | 기본값(현행)                                                 | 비고                                                                                       |
| --- | ------------ | -- | --------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| A1  | 인증 방식        | ⚠️ | JWT (access 짧은 수명 + refresh) + 소셜 OAuth                   | JWT access+refresh는 **운영 중**(회전·플랫폼 분기 포함). refresh 전달: Web = `Set-Cookie`(httpOnly), Native = 응답 본문, `X-Client-Platform`로 구분(§2). **소셜 OAuth는 아직 Mock**(`/auth/login/{provider}`, §0.3·§2) |
| A2  | 비동기 처리 모델    | ✅ | **리소스 내 상태 필드 + 클라이언트 폴링**(1~2s)                          | `/jobs` 서브시스템 없이 리소스 GET으로 상태 확인 — 운영 중. **SSE(§7)는 미구현·선택 확장**(폴링이 기본) |
| A3  | 자료(PDF·PPTX) 파싱 | ✅ | 비동기 (`material.status`)                                   | BackgroundTasks 파싱(PyMuPDF·python-pptx) + 재시도/재업로드 운영 중                                    |
| A4  | STT (발표·답변)  | ✅ | 비동기 (`transcript.status`, `answer.status`)               | 원문(raw) 전사만 수행 — LLM 정제 패스 없음(v0.3). 직렬 STT 큐 운영. 타임스탬프는 ForcedAligner 산출               |
| A5  | AI 질문 생성     | ✅ | 비동기 (`session.status = generating_questions`)            | STT 의존이라 비동기로 통일 — 운영 중                                                                   |
| A6  | TTS (질문 음성)  | ✅ | 비동기 (`question.tts.status`)                               | self-hosted VoxCPM2로 **질문별 인라인 합성** 운영 중. 동시성 SLA·페르소나 음성 매핑은 §8에서 후속                     |
| A7  | 분석 리포트       | ✅ | 비동기 + **세션 종료 시 자동 생성**                                   | `qna/end`·마지막 질문 pass 시 자동 생성. `completed` 전에는 `report` 부재(§4.1)                          |
| A8  | 발표 녹음 방식     | ✅ | 클라이언트가 로컬 녹음 → **종료 시 파일 업로드**(일괄). 파일/실시간 모드가 업로드 지점에서 수렴 | 데모 기본은 일괄 업로드(§0.8). 실시간 청크(`/recording/chunks`·`/complete`)는 **서버 구현 완료**이나 FE 수렴은 후순위(§4.3.1) |
| A9  | 타이머          | ✅ | 클라이언트 권위. 서버는 `started_at/ended_at`만 저장                   | 녹음 업로드 multipart로 수신·저장 — 운영 중                                                            |
| A10 | 파일 저장        | ⚠️ | 서명 URL(`*_url`)로 재생 (§1.4)                                | **현재 로컬 파일시스템 + HMAC 서명 URL로 운영**(§1.4). 오브젝트 스토리지 전환·보관(retention) 정책은 후속(§8)             |
| A11 | 꼬리질문 최대 깊이   | ✅ | **1**                                                     | `follow_up_depth ≤ 1` 강제 — 운영 중                                                          |
| A12 | 질의응답 종료 우선순위 | ✅ | 사용자 종료 > 질의 수 도달 > 답변 시간초과                                | `ended_reason`(user_end·count_reached·timeout)으로 확정 — 운영 중                               |
| A13 | 마이크 권한       | ✅ | **클라이언트 전용**(API 없음)                                      | 브라우저/OS 권한 — 해당 API 없음(설계상 N/A)                                                           |
...
### 0.6 변경 이력 (v0.3 → v0.4-draft) — 실시간 녹음 청크 계약 (제안 → §0.8에서 합의 완료)

README 아키텍처("녹음 중 60초+4초 겹침 청크 전송", STT 실측으로 청크 크기 확정)에 대응하는 API 계약이 spec에 없어 FE(팀원1)가 초안 추가. **합의 결과(§0.8): 데모까지는 일괄 업로드(A8)로 수렴하고, 청크 계약은 후순위 순수 최적화로 §4.3.1에 draft 보존.**
...
### 0.8 변경 이력 (v0.4-draft) — recording 청크 3종 정합 합의 (남은 일 #4, 합의 완료)

스펙(§4.3.1 청크 계약) · FE(실시간 모드가 청크 API 호출) · BE(일괄 업로드만 구현) 3자 불일치를 합의로 해소. **결정: 데모까지는 일괄 업로드(A8) 하나로 수렴한다.**

**밸런싱 원칙 — 중복이 아니라 계층이다.** STT를 위한 분할은 **BE 소유(필수·불변)**, 전송을 위한 분할은 **FE 소유(선택적 최적화)**. 안정된 계약은 `GET /transcript` 폴링 하나이며, 오디오가 서버에 어떻게 도착했는지(일괄/청크)는 그 계약 뒤에 숨는다. FE 청크는 "있으면 빨라지고 없어도 되는" 레이어, BE `stt.py`는 어느 경로로 왔든 동일하게 쓰이는 부품.

**데모까지 A8 수렴 근거(숫자):**

| 근거 | 내용 |
|---|---|
| RTF 실측 | STT RTF ≈ 0.03 → 10분 발표 전사 ≈ 20초. 데모 규모(5~10분)에서 청크로 버는 시간은 20~30초뿐이고, 이미 만든 processing 화면 + 폴링이 그 구간을 덮음. 청크의 진짜 가치는 60분 녹음(~2분 대기)에서 발생 |
| FE 수정 최소 | `_startRecording`에서 `startRecording` 호출 제거, `_enqueueChunk`를 no-op으로, `_finish`에서 `completeRecording` 대신 기존 `uploadRecording`(= `POST /recording`)으로 전체 WAV 업로드. `PcmChunker`는 전체 WAV 축적용(`buildFullWav`)으로 존치. 상태 머신 이미 `draft → transcribing`(파일 업로드) 전이 보유 |
| 전송량 감소 | 현재 FE는 청크(전체 분량) + complete(전체 파일)로 오디오를 2번 전송 중 → 일괄 1회로 줄어듦 |
| 경로 단일화 | 권한 거부/미지원 폴백(파일 업로드)이 이미 일괄 라우트를 쓰므로, 실시간까지 수렴하면 테스트 경로가 하나 |

| 항목 | 변화 |
|---|---|
| `POST /recording` | **데모 기본·유일 경로** — 실시간/파일 모드가 여기로 수렴. BE 무변경 |
| `POST /recording/chunks` · `/recording/complete` | **후순위 순수 최적화로 강등** — §4.3.1에 draft 보존, 데모 미구현. 여유 시 얹음 |
| FE 실시간 모드 | 청크 파이프라인 헛돌던 것을 일괄 업로드로 수렴(소폭 수정, 팀 합의 후 즉시 반영 가능) |
| 상태 머신·enum | 변경 없음 |

**§4.3.1을 나중에 살릴 때 챙길 4가지(§4.3.1 하단 참조):** ① 청크 멱등성, ② `complete`는 청크 완비를 요구하지 않음, ③ 겹침 방향 함정(FE는 앞 겹침, `stt.py`는 뒤 겹침), ④ 청크 STT 결과 저장소(JSONB). 상세는 §4.3.1.

### 0.9 변경 이력 (v0.4-draft) — 이메일 인증 강제 + 초대코드 통일 (2026-07-14 팀 결정, 합의 완료)

상세 설계·작업 계획: [docs/backend-core/email-verification-plan.md](backend-core/email-verification-plan.md). 예약만 돼 있던 이메일 인증 엔드포인트의 계약을 확정하고, 미인증 유저의 로그인을 차단한다.

| 항목 | 변화 |
|---|---|
| 로그인 차단 | 미인증 유저의 `POST /auth/login` → **`403 EMAIL_NOT_VERIFIED`** (비밀번호 검증 통과 후에만 — 실패는 기존 401). 가입→인증→로그인이 필수 순서가 됨 (§2) |
| `/auth/email/verify-request` | 응답 확정: **204**(유저 없음·이미 인증도 204 — 계정 열거 방지), 60초 쿨다운 위반 시 `429 RATE_LIMITED`+`Retry-After` (§2) |
| `/auth/email/verify` | 응답 확정: **200** `{ "email_verified": true }`(멱등) · `400 INVALID_CODE`/`CODE_EXPIRED`. 코드는 6자리 숫자, TTL 10분, 5회 실패 시 소진 (§2) |
| 에러 코드 | `INVALID_CODE`(400) · `CODE_EXPIRED`(400) · `RATE_LIMITED`(429) 추가 (§6.2) |
| 발송 수단 | Gmail SMTP(운영) / mock(개발·테스트 기본, `EMAIL_PROVIDER=mock\|smtp`) — 서버 내부 구성, API 계약 아님 |
| ⚠️ 배포 주의 | 기존 가입자 전원이 미인증 상태 → **신 코드 배포 직전 일괄 인증 SQL 필수**(plan §7-2). 빼먹으면 전원 로그인 불가 |
| 초대코드 통일 | 링크 초대 `token`을 8자 초대코드로 발급(§3.1). API 계약 변경 없음. 이메일 초대는 미사용(스코프 컷) 표기 |

### 0.10 변경 이력 (v0.4-draft) — 구현 정합 반영 (2026-07-15)

구현된 백엔드와 문서가 어긋났던 지점을 문서 쪽으로 맞췄다. **엔드포인트 신설/제거 없음** — 상태 enum·응답 예시·이미 존재하던 파일 서빙 경로의 문서화만 갱신.

| 항목 | 변화 |
|---|---|
| `QnaStatus`에 `failed` 추가 | `GET /qna`는 질문 생성이 실패하면 `status: "failed"`를 내려준다(빈 `questions[]` 가능) → FE가 재생성 안내. 구현이 이미 이렇게 동작(§4.4·§6.1) |
| 리포트 `over_time` 제거 정합 | §5.2 예시에서 `speaking_habits.over_time` 삭제 — §5.2 주석대로 클라이언트 파생이라 응답에 없음(예시만 뒤늦게 정합) |
| 서명 URL 파일 서빙 문서화 | 모든 `*_url` 재생을 담당하는 `GET /files/{key}?expires=&sig=`를 §1.4로 명문화(신설 아님, 원래 구현됨) + 에러코드 `URL_EXPIRED`/`INVALID_SIGNATURE`(§6.2) |
| 소셜 로그인 | 실구현은 아직 Mock(`POST /auth/login/{provider}`). 최종 계약 경로 `/auth/login/social/{provider}`(§2)는 실구현 시 교체 — 스펙은 의도 경로 유지 |
| §0.2 기본값 현행화 | "가정값(⚠️ 확정 필요)" → **"현재 구현·운영 중"**으로 재프레이밍. `현황` 열 추가(A1~A13), 잔여 확정은 A1(소셜 Mock)·A10(로컬 FS→오브젝트 스토리지·보관 정책)뿐임을 명시 |
...
### 1.4 파일 재생 — 서명 URL (A10)

모든 재생용 `*_url`(`recording.audio_url`, `question.tts.audio_url`, `answer.audio_url`)은 아래 서명 URL이다. 리소스를 GET할 때 서버가 발급하며, **클라이언트는 URL을 조립하지 않고 받은 값을 그대로 재생**한다.

| Method | Path | 인증 | 설명 |
| ------ | ---- | -- | ---- |
| GET    | `/files/{key}?expires=<unix>&sig=<hmac>` | ✗(서명) | 서명·만료가 유효할 때만 파일 반환. Bearer 토큰 불필요 — **링크 자체가 접근권**. `Range`(206 부분 응답) 지원(iOS AVPlayer 오디오 스트리밍) |

- 만료 `410 URL_EXPIRED` / 서명 불일치 `403 INVALID_SIGNATURE` / 부재 `404 FILE_NOT_FOUND`.
- 만료 시 클라이언트는 원 리소스를 다시 GET해 새 `*_url`을 받는다(URL 수명 < 세션 수명).
...
> **초대코드 통일(v0.4-draft, 2026-07-14 팀 결정 — §0.9):** 링크 초대의 `token`은 **8자 초대코드**(대문자+숫자, 혼동 문자 `I O 0 1` 제외 32자 알파벳)로 발급한다. 엔드포인트·응답 스키마는 **변경 없음** — `token`이 짧아질 뿐이며 `url`은 웹 딥링크 겸용으로 유지, 기존 발급된 긴 토큰도 계속 유효. 프론트의 초대 UX는 초대코드(표시·복사·입력)로 통일한다.

**이메일 초대** — ⚠️ **미사용(스코프 컷)**: 메일 발송 없음·프론트 미사용. 엔드포인트는 존치.
...
#### 4.3.1 실시간 녹음 청크 파이프라인 (후순위 최적화 draft — 데모 미구현)

> **상태(§0.8 합의):** 데모까지는 §4.3의 일괄 업로드(`POST /recording`) 하나로 수렴한다. 이 절은 **순수 최적화** 계약으로 보존하며, 여유가 생겼을 때 BE에 얹는다. 아키텍처 설계 자체(seq/offset/overlap 메타 + `complete` 전체 파일 안전망)는 그대로 유효하다. 구현 시 아래 **"살릴 때 챙길 4가지"**를 반드시 반영한다.
...
**이 절을 살릴 때 챙길 4가지 (§0.8 합의 — 구현 전 필독)**

| # | 포인트 | 계약 |
|---|---|---|
| ① | **청크 멱등성** | 같은 `seq` 재전송은 덮어쓰고 동일한 `202 { received_seq }`를 반환. FE 재시도가 공짜가 된다 |
| ② | **`complete`는 청크 완비를 요구하지 않음** | 수신된 `seq`의 STT 결과는 재사용, 누락 구간만 `complete`의 전체 파일에서 잘라 재전사. 이 폴백 덕에 청크가 **전부 유실돼도** 일괄 경로와 동일하게 동작(§4.3.1 폴백 안전망과 동일 취지) — `complete`가 "수신 seq ⊂ 0..total_chunks−1" 완비를 **강제하지 않도록** 위 문단의 검증 문구를 완화한다 |
| ③ | **겹침 방향 함정** | FE 청크는 겹침이 **앞**에 붙고(청크 i = `[60i−4, 60(i+1))`), `stt.py`는 겹침이 **뒤**에 붙는다(청크 i = `[60i, 60i+64)`). 팀원2가 `_shift_and_trim`의 `60·4` 하드코딩 산식을 그대로 재사용하면 절단점이 4초 어긋난다. FE가 이미 보내는 `offset_seconds`/`overlap_seconds` 메타데이터로부터 `lo`/`hi` 창을 **계산**하게 하면 양쪽 규약을 모두 수용 |
| ④ | **청크 STT 결과 저장소** | `seq`별 STT 결과를 `transcript`에 **JSONB**로 쌓고 `complete`에서 병합. 직렬 STT 큐는 그대로 공유(발표 중엔 답변 STT가 없어 경합 없음, infra 제약 2) |
...
```
{
  "status": "in_progress",       // in_progress | ended | failed
...
- **질문 생성 실패**: 생성 단계에서 실패하면 `GET /qna`가 `status: "failed"`로 내려온다(`questions[]`는 빈 채일 수 있음). FE는 이를 신호로 `POST /qna/generate`를 재호출해 재생성한다(§6.1 `QnaStatus.failed`).
...
```
{
  ...
  "speaking_habits": {
    ...
    "time_limit_seconds": 600, "actual_seconds": 663
  },
...
| `QnaStatus`         | `in_progress · ended · failed`(`failed`=질문 생성 실패 → `GET /qna`가 노출, 재생성 유도)                              |
...
| `CODE_EXPIRED`                                    | 400     | 이메일 인증코드 만료·5회 소진·없음 (→재발송 유도) |
| `RATE_LIMITED`                                    | 429     | 인증코드 재발송 60초 쿨다운 등 (`Retry-After` 헤더 참조) |
| `INVALID_SIGNATURE`                               | 403     | 파일 서명 URL 서명 불일치 (§1.4)  |
| `URL_EXPIRED`                                     | 410     | 파일 서명 URL 만료 (→리소스 재조회로 새 URL, §1.4) |
