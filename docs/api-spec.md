...
작성일: 2026-07-11 · 개정: 2026-07-11 (v0.2) · 상태: **초안(가정 기반)** — §0.2 가정값은 팀 논의 후 확정 필요.
...
| A4  | STT (발표·답변)  | 비동기 (`transcript.status`, `answer.status`)               | **답변 STT 상태 필드명은 `answer.status`로 통일** (A→I 정합)                                            |
...
- **TTS 다시 듣기**: 동일 `audio_url` 재생(새 요청 불필요). `tts.status = failed` 시 질문 상세를 폴링하며, 재생성이 필요하면 별도 협의(후속).
- **답변 자동 녹음**: 질문 `tts.status = ready` 도달 후 클라이언트가 자동 녹음 시작. API는 결과 오디오만 수신.

> **종료 조건**(A12): 사용자 `qna/end` > 설정 질의 수(1차 질문 기준) 도달 > 답변 시간초과(→ 자동 다음). 시간초과는 클라이언트가 감지해 `pass` 또는 빈 답변 제출로 처리하고, 서버가 `ended_reason`을 확정.
