...
- **입력**: 발표 제목 · **slides**(`materials.slides` = `[{page,text}]`) · **transcript**
  (`transcripts.segments` = `[{start,end,text}]`) · **세션 선택 personas**(≥1, 5종) · 질문 수
- **출력**: 질문 초안 목록. 각 초안 =
  ```
  { text, persona, strategy, evidence: { slides: [int], transcript_refs: [{start: float}] } }
  ```
  ...
  - `evidence.slides` = 근거 슬라이드 `page` 번호, `transcript_refs` = 초 단위 `start`(API가 `ts:"MM:SS"`로 포맷 — db-schema §6.3)
- **타게팅(핵심)**: slides와 transcript를 **대조**해
...
| 3 | **evidence 미생성** | 없음 | `{slides:[page], transcript_refs:[{start}]}` | 근거 배지(FE)·리포트 근거 추적 불가 |
...
- [ ] slides(페이지 번호 유지)와 transcript(ts 유지)를 **번호 붙여** 프롬프트에 주입 — 모델이 evidence로 되짚게
- [ ] 출력 강제: `response_schema`에 persona(세션 personas로 enum 제한)·strategy·evidence 포함.
  persona는 프롬프트에서 "선택된 목록 안에서만, 고르게 배분"으로 지시 + 코드에서 라운드로빈 보정
- [ ] 타게팅 규칙 문장화: "슬라이드에 있으나 transcript에 없음 / 있으나 근거·수치 약함"을 우선순위로
- [ ] evidence 사후검증: `slides`는 실제 `page` 범위 안, `transcript_refs.start`는 실제 세그먼트 근처로 클램프
  (환각 근거 방지 — 범위 밖이면 빈 배열로 강등)
