...
class ApiClient {
  ...
  /// 앱 시작 시 저장된 세션을 복원한다 (새로고침 후에도 로그인 유지).
  ///
  /// 새로고침하면 메모리의 access 토큰은 사라지지만, refresh는 살아있다
  /// (Web=httpOnly 쿠키 / Native=secure storage). 이 메서드는 그 refresh로
  /// `/auth/refresh`를 한 번 호출해 새 access 토큰을 메모리에 채운다.
  ///
  /// 반환값: 복원 성공 여부. `false`면 재로그인이 필요하다(쿠키 없음·만료).
  /// 성공 후 호출자는 `/auth/me`로 사용자 정보를 다시 받아오면 된다.
  Future<bool> tryRestoreSession() => _tryRefresh();
