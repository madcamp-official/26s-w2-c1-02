...
class MockLLMProvider(LLMProvider):
    async def generate_questions(
        ...
        *,
        ...
        first_start = (transcript[0].get("start") if transcript else None)
        ...
        for i in range(count):
            drafts.append(QuestionDraft(
                ...
                evidence=Evidence(
                    slides=[first_page] if first_page is not None else [],
                    transcript_refs=[TranscriptRef(start=float(first_start))] if first_start is not None else [],
                ),
            ))
        return drafts
