...
3. **TTS 음성(voice) 파라미터**: 페르소나 5종(에겐/테토/꼰대/멍청/잼민) **셀프 캐스팅 음색 배포 중**(2026-07-13) — `voice="egen"` 등으로 200 반환. VoxCPM2 `default`가 요청마다 화자를 샘플링한다는 실측을 이용해, N회 샘플 중 팀 청취 픽으로 5종을 선발·클로닝(`build_persona_refs_voxcpm.py`, 출력 실측 CER 0%·최소 MFCC 거리 137). 백엔드는 `persona→voice` 1:1 매핑 후 실패 시 `default` 폴백(변화 없음). **참고:** 방향 제어(꼰대 저음 등)는 "다른 화자/포먼트-정합 레퍼런스면 클로닝을 통과"함이 확인됐으나(edge-tts 실측), 현행 세트는 자연스러움 우선 청취 픽이라 설계 서열과 다름. 단 ref F0가 항상 전이되진 않으므로 레퍼런스 교체 시 **출력** 실측 필수(`eval_persona_voices.py`). 라우트 이력·실측·절차: [persona_voices.md](persona_voices.md).
