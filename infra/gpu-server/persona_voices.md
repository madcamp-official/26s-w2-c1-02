...
## Path B — 부트스트랩 레퍼런스 (사람 녹음 전 중간 단계, 2026-07-13)

현재 refs는 캐리커처(default 합성 + 피치/템포 변형)라 **5종 모두 같은 인공 음색의
변형**이다. 사람 녹음(Path A) 전까지 사실감을 끌어올리는 중간 단계로, 지시 기반 TTS로
레퍼런스를 다시 만든다. 산출은 별도 스테이징 디렉터리(작동 중인 refs를 덮지 않음)에
두고, 청취 검수 후 교체→(3)단계 재계산→재기동. voice 이름 불변 → 백엔드 수정 0.

> **현재 상태(2026-07-13):** 아래 MOSS 라우트로 뽑은 스테이징 레퍼런스는 정확도가
> 낮아 채택하지 않았다. **live 소스는 `voices/refs/`로 유지**한다. 스크립트는
> 재실험용으로 남겨 두되, 검수 통과 전에는 `refs/`로 승격하지 않는다.

### 로컬 MOSS-VoiceGenerator (무료·키 불필요 — 결과: 폐기, 아래 참조)
...
한계: 합성음 재합성 구조라 Path A(사람 녹음)보다 사실감 상한은 낮다.
