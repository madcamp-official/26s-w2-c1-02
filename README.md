...
> **불변 조건:** 청크 STT 처리시간 < 청크 길이여야 60분 발표에서 백로그가 쌓이지 않는다. **Day 3 실측(2026-07-11)으로 청크 길이 = 60초 + 4초 겹침 확정** — 60초 청크 전사 ~2.1s로 처리시간이 청크 길이의 ~1/30(RTF≈0.03~0.05), 5분 오디오도 <10s라 여유가 크다. 답변 오디오는 짧으므로 청크 없이 단발 업로드. 실측표·근거: [infra/gpu-server README](infra/gpu-server/README.md#stt-실측--청크-크기-결정-day-3-팀원3).
...
- **무료 LLM API의 일일 호출량 한도가 개발·시연을 따라오지 못함.** Gemini 무료 티어는 모델당 하루 20회(RPD) 수준이라, 질문·꼬리질문·리포트를 반복 테스트하다 보면 오후에 쿼터가 소진되어(429) 핵심 기능이 통째로 멈추는 날이 있었다. 모델·키 폴백 체인으로 버텼지만 이는 임시방편이고, 예산이 없어 유료 티어로 올리지 못한 것이 근본 제약이었다. 개발용 호출(프롬프트 튜닝·E2E 테스트)이 시연용 쿼터를 갉아먹는 구조도 문제 — 쿼터 소비를 아끼는 mock 활용·캐싱 전략을 더 일찍 세웠어야 했다.
- **VM(malggori)이 원인 불명으로 죽어 다른 VM(horsetail)으로 이전하는 소동을 겪음.** 재배포 자체는 빨랐지만(Keep 참고), 장애를 모니터링이 아니라 사람이 접속해 보고서야 알았고, DB·업로드 파일이 VM 한 대의 로컬 디스크에만 있는 단일 장애점 구조라 최악의 경우 데이터를 통째로 잃을 수 있었다. 헬스체크 알림·주기적 DB 덤프·파일 백업 같은 최소한의 안전장치 없이 단일 VM에 모든 것을 올려둔 점이 아쉬웠다.
- **소셜 로그인(OAuth)을 구현하지 못하고 이메일 인증 기반 자체 로그인만 제공함.** SMTP 실발송 이메일 인증·아이디 찾기·비밀번호 재설정까지는 완성했지만, Google 등 소셜 로그인이 없어 가입 시 인증 코드 확인을 거쳐야 하는 진입 장벽이 남았다. OAuth는 리다이렉트 URI·플랫폼별(웹/모바일) 처리 등 초기 설정 비용이 커서 일정상 후순위로 밀렸는데, 인증은 다른 기능의 전제가 되는 만큼 초반에 범위를 정해 함께 설계했어야 했다.

### Try — 다음번에 시도해볼 것

- **소셜 로그인(OAuth) 구현하기.** Google 로그인부터 시작해 가입 진입 장벽을 낮춘다. 리다이렉트 URI·웹/모바일 플랫폼별 처리 등 초기 설정 비용이 큰 작업이므로, 다음엔 후순위로 미루지 말고 프로젝트 초반 인증 설계 단계에 범위를 확정해 함께 구현한다.
- **유료 LLM API 티어 도입 검토하기.** 무료 티어 쿼터(모델당 20 RPD)가 개발·시연 병목이었으므로, 다음엔 예상 호출량을 먼저 산정해 소액이라도 유료 티어를 쓰는 것을 검토한다. 유료 전환이 어렵다면 최소한 개발용 호출은 mock·응답 캐싱으로 돌려 시연용 쿼터를 지키는 체계를 초반부터 갖춘다.
- **헬스체크 알림·데이터 백업 등 최소 안전장치 갖추기.** VM 장애를 사람이 발견하는 대신 주기적 헬스체크 + 실패 시 알림(Slack/Discord 웹훅 등)으로 자동 감지하고, DB 덤프·업로드 파일을 다른 곳에 주기 백업해 단일 VM 장애가 데이터 유실로 이어지지 않게 한다.
